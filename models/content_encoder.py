...
# https://github.com/gmltmd789/UnitSpeech/blob/master/unitspeech/encoder.py
...
class ConvReluNorm(BaseModule):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size,
        ...
        for _ in range(n_layers - 1):
            self.conv_layers.append(torch.nn.Conv1d(hidden_channels, 
                                                    hidden_channels,
                                                    kernel_size, padding=kernel_size//2))
            self.norm_layers.append(LayerNorm(hidden_channels))
...
class Encoder(BaseModule):
    def __init__(
            self, n_contentvec, n_feats, n_channels,
            n_out, filter_channels,
        ...
        self.encoder = EncoderModule(n_channels,
                                     filter_channels,
                                     n_heads,
                                     n_layers,
                                     kernel_size,
        ...
        self.proj_x = torch.nn.Conv1d(n_channels, n_out, 1)
    ...
    def forward(self, x):
        ...
        x = self.prenet(x)
        x = self.encoder(x)
        mu_x = self.proj_m(x).transpose(1, 2)
        x = self.proj_x(x).transpose(1, 2)
