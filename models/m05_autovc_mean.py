...
class M05AutoVCMean(nn.Module):
    """
    """
    def __init__(self, h):
        super(M05AutoVCMean, self).__init__()
        ...
        # 2-2. Quantizer
        h_spk_m_q = h.SpeakerMeanQuantizer
        max_n_q, nbins = self._calculate_q_params(h_spk_m_q)
        print("SPK_mean: max_n_q:{}, nbins:{} => {} kbps".format(max_n_q, nbins, h_spk_m_q.target_bandwidths[0]))
       
        self.spk_m_fr = h_spk_m_q.frame_rate
        self.spk_m_bw = h_spk_m_q.target_bandwidths[-1]

        self.spk_mean_quantizer = ResidualVectorQuantizer(
                        dimension=h_spk_m_q.dimension,
                        n_q=max_n_q,
                        bins=nbins,
                        decay=h_spk_m_q.decay,
                        )

        h_spk_r_q = h.SpeakerResidualQuantizer
        max_n_q, nbins = self._calculate_q_params(h_spk_r_q)
        print("SPK_res: max_n_q:{}, nbins:{} => {} kbps".format(max_n_q, nbins, h_spk_r_q.target_bandwidths[0]))
       
        self.spk_r_fr = h_spk_r_q.frame_rate
        self.spk_r_bw = h_spk_r_q.target_bandwidths[-1]

        self.spk_res_quantizer = ResidualVectorQuantizer(
                        dimension=h_spk_r_q.dimension,
                        n_q=max_n_q,
                        bins=nbins,
                        decay=h_spk_r_q.decay,
                        )
    ...
    def forward(self, x, quantize=[1, 1], centroid_dict=None, inference=False, **kwargs):
        ...
        if 'utt_c' in kwargs.keys():
            spk_enc_mean = kwargs['utt_c']
        else:
            spk_enc_mean = torch.mean(spk_enc, dim=-1).unsqueeze(-1)
        spk_enc_res = spk_enc - spk_enc_mean.expand(spk_enc.size())

        # 2-1. Mean pooling and quantize
        if not inference:
            spk_mean_q_result = self.spk_mean_quantizer.forward(spk_enc_mean, sample_rate=self.spk_m_fr, bandwidth=self.spk_m_bw)
            q_spk_enc_mean = spk_mean_q_result.quantized
        else:
            spk_mean_codes = self.spk_mean_quantizer.encode(spk_enc_mean, sample_rate=self.spk_m_fr, bandwidth=self.spk_m_bw)
            q_spk_enc_mean = self.spk_mean_quantizer.decode(spk_mean_codes)

        # 2-2. Quantize residual
        if not inference:
            spk_res_q_result = self.spk_res_quantizer.forward(spk_enc_res, sample_rate=self.spk_r_fr, bandwidth=self.spk_r_bw)
            q_spk_enc_res = spk_res_q_result.quantized
        else:
            spk_res_codes = self.spk_res_quantizer.encode(spk_enc, sample_rate=self.spk_r_fr, bandwidth=self.spk_r_bw)
            q_spk_enc_res = self.spk_res_quantizer.decode(spk_res_codes)

        q_spk_enc = q_spk_enc_mean + q_spk_enc_res
        spk_dec = self.spk_refiner.decode(q_spk_enc)
        feat_dict['spk_raw'] = spk_raw
        feat_dict['spk_enc_mean'] = spk_enc_mean
