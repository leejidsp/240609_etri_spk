...
class M03MultipleVQ(nn.Module):
    """
    """
    def __init__(self, h):
        super(M03MultipleVQ, self).__init__()
        ...
        self.spk_quantizer_list = []
        for i in range(h_spk_q.vocab_size):     
            self.spk_quantizer_list.append(ResidualVectorQuantizer(
    ...
    def forward(self, x, quantize=[1, 1], centroid_dict=None, inference=False, **kwargs):
        ...
        B, C_enc, T = spk_enc.size()

        spk_enc_reshape = spk_enc.transpose(1, 2).reshape(-1, C_enc)    # (B*T, C_enc)
        q_spk_enc = torch.zeros_like(spk_enc_reshape)

        for i in range(int(B*T)):
            spk_enc_i = spk_enc_reshape[i].reshape(1, -1, 1)
            argi = argindices[i]
            if not inference:
                spk_q_result = self.spk_quantizer_list[argi].forward(spk_enc_i, sample_rate=self.spk_fr, bandwidth=self.spk_bw)
                q_spk_enc[i] = spk_q_result.quantized.squeeze()
            else:
                spk_codes = self.spk_quantizer_list[argi].encode(spk_enc_i, sample_rate=self.spk_fr, bandwidth=self.spk_bw)
                q_spk_enc[i] = self.spk_quantizer_list[argi].decode(spk_codes).squeeze()

        q_spk_enc = q_spk_enc.reshape(B, T, C_enc).transpose(1, 2)  # (B, C_enc, T)
