...
class TrainerHiFiBasic(object):
    ...
    def _train_one_epoch(self, epoch):
        ...
        for path_list in pbar:
            ...
            # ===== Main Part =====
            # For each filebatch
            for i, (x, _, _) in enumerate(train_frame_loader):
                ...
                gen_x, feat_dict = self.coder(real_x, [1, 1], self.centroid_dict, iteration=self.iteration)  # (B, 1, T)
    ...
    def _infer_frames_from_multiple_files(self, f_frame_list, wavlen_list, q_tag):
        ...
        for fr_i in range(max_num_frame):
            ...
            # Infer
            with torch.no_grad():
                stacked_gen_frame = self.coder.inference_tag(stacked_frame, q_tag, self.centroid_dict, iteration=self.iteration)
