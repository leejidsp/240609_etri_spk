...
class LossReconMSDMPDContrastive(nn.Module):
    def __init__(self, loss_list, loss_weight, params, **kwargs):
        super(LossReconMSDMPDContrastive, self).__init__()
    ...
    def _update_loss_dict(self, name, loss_group):
        ...
        elif name in ['cs_spk_p', 'cs_spk_n']:
            loss, w_loss = loss_group

            self.losses_iter[name] = loss.item()
            
            self.losses_tb['cs']['original'][name] = loss.item()
            self.losses_tb['cs']['weighted'][name] = w_loss.item()


    # --- Constrastive ---------------
    def contrastive_loss(self, x, p_x, n_x, zero=False):

        if zero:
            loss_cs_p   = torch.zeros(1).cuda().sum()
            w_loss_cs_p = torch.zeros(1).cuda().sum()
            loss_cs_n   = torch.zeros(1).cuda().sum()
            w_loss_cs_n = torch.zeros(1).cuda().sum()
            total_loss  = torch.zeros(1).cuda().sum()
        else:
            m_x = torch.mean(x, dim=-1)
            m_p_x = torch.mean(p_x, dim=-1)
            m_n_x = torch.mean(n_x, dim=-1)

            loss_cs_p = -1 * self.cs(m_x, m_p_x).mean()
            w_loss_cs_p = loss_cs_p * self.loss_weight['cs_spk_p']
            
            loss_cs_n = self.cs(m_x, m_n_x).mean()
            w_loss_cs_n = loss_cs_n * self.loss_weight['cs_spk_n']

            total_loss = w_loss_cs_p + w_loss_cs_n

        self._update_loss_dict('cs_spk_p', (loss_cs_p, w_loss_cs_p))
        self._update_loss_dict('cs_spk_n', (loss_cs_n, w_loss_cs_n))
